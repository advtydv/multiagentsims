...
def batch_dashboard():
    ...
    return render_template('batch_enhanced.html')

@app.route('/api/simulations')
def get_simulations():
    """Get all single simulations (including those within batches)"""
    simulations = []
...
def get_batch_summary(batch_id):
    ...
    import sys
    sys.path.append('.')
    from simple_batch_processor import process_simple_batch
    ...
    try:
        # Use simple batch processor
        summary = process_simple_batch(str(batch_dir))
...
@app.route('/api/batch/<batch_id>/real')
def get_real_batch_summary(batch_id):
    """Get real aggregated summary using the new processor"""
    import sys
    sys.path.append('.')
    from real_batch_processor import process_batch_data
...
@app.route('/api/batch/<batch_id>/enhanced')
def get_enhanced_batch_summary(batch_id):
    """Get enhanced batch analysis with meaningful metrics"""
    import sys
    sys.path.append('.')
    ...
    for sim_dir in sorted(batch_dir.iterdir()):
        if sim_dir.is_dir() and sim_dir.name.startswith('simulation_'):
            ...
            if results_file.exists():
                ...
                final_revenue_board = results.get('final_revenue_board', {})
                if agent_id in final_revenue_board:
                    sim_data = {
                        'simulation': sim_dir.name,
                        'final_revenue': final_revenue_board[agent_id],
                        'final_position': list(final_revenue_board.keys()).index(agent_id) + 1
                    }
    ...
    for sim_dir in sorted(batch_dir.iterdir()):
        if sim_dir.is_dir() and sim_dir.name.startswith('simulation_'):
            ...
            if results_file.exists():
                ...
                # Get winner safely
                revenue_board = results.get('final_revenue_board', {})
                winner = list(revenue_board.keys())[0] if revenue_board else None
