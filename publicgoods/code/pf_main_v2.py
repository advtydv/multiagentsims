...
# Removed update_parameters function - now using config directly
...
def print_config_summary():
    ...
    print(f"   Crisis: {config.CRISIS_MULTIPLIER}x multiplier")
    print(f"   Token Range: {config.MIN_TOKENS_PER_ROUND} (rep 0) → {config.MAX_TOKENS_PER_ROUND} (rep 10)")
    print(f"   Pool Failure: < {config.POOL_FAILURE_THRESHOLD*100}% contributions")
...
def main():
    ...
    # No longer needed - using config directly
